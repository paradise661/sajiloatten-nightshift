    ...
    <div class="xl:col-span-12 col-span-12 mt-4">
        <div class="box rounded-lg shadow-lg border border-gray-200">
            ...
            <div class="box-body p-4">
                <form class="grid grid-cols-12 gap-6 mt-0" action="{{ route('shifts.update', $shift->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-span-12">
                        ...
                        <input class="form-control @error('name') border-red-500 @enderror" type="text" name="name"
                            placeholder="Enter Name" value="{{ old('name', $shift->name) }}" autocomplete="off">
                        @error('name')
                    ...
                    <div class="col-span-12">
                        <label class="inline-flex items-center gap-2">
                            <input id="is_cross_day" type="checkbox" name="is_cross_day"
                                {{ old('is_cross_day', $shift->is_cross_day) ? 'checked' : '' }}>
                            <span class="text-sm font-medium">Is Cross Day?</span>
                        </label>
                        <p class="text-xs text-gray-500 mt-1">
                            Check this box if the shift period extends into the next day. (e.g., night shift from 10 PM
                            to 6
                            AM next day).
                        </p>
                    </div>

                    <div class="col-span-6 {{ $shift->is_cross_day ? 'hidden' : '' }}" id="day_end_time_wrapper">
                        ...
                        <div class="input-group relative">
                            ...
                            <input class="form-control timepicker @error('day_end_time') border-red-500 @enderror"
                                ...
                                value="{{ old('day_end_time', $shift->day_end_time ?? '23:59:59') }}"
                                placeholder="Enter Day End Time" autocomplete="off">
                        </div>
                        <p class="text-xs text-gray-500 mt-1">
                            ...
                            <table
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @php
                                        ...
                                        $desc = json_decode($shift->description, true) ?? [];
                                    @endphp

                                    @foreach ($days as $day)
                                        @php
                                            $dayKey = $day;
                                            $savedStart = $desc[$dayKey]['start_time'] ?? '10:00';
                                            $savedEnd = $desc[$dayKey]['end_time'] ?? '18:00';
                                            $savedHoliday = $desc[$dayKey]['is_holiday'] ?? $dayKey === 'Saturday';
                                            $oldHoliday = old($dayKey . '_holiday');
                                            $isHolidayChecked = is_null($oldHoliday) ? $savedHoliday : true;
                                            $startTime = $isHolidayChecked
                                                ? ''
                                                : old($dayKey . '_start_time', $savedStart);
                                            $endTime = $isHolidayChecked ? '' : old($dayKey . '_end_time', $savedEnd);
                                        @endphp
                                            ...
                                            <td class="py-3 px-2">
                                                <div class="input-group relative">
                                                    ...
                                                    <input
                                                        ...
                                                        name="{{ $dayKey }}_start_time" value="{{ $startTime }}"
                                            ...
                                            <td class="py-3 px-2">
                                                <div class="input-group relative">
                                                    ...
                                                    <input
                                                        ...
                                                        name="{{ $dayKey }}_end_time" value="{{ $endTime }}"
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Handle cross day toggle
            $('#is_cross_day').on('change', function() {
                if ($(this).is(':checked')) {
                    $('#day_end_time_wrapper').show();
                    $('#day_end_time').prop('disabled', false);
                } else {
                    $('#day_end_time_wrapper').hide();
                    $('#day_end_time').prop('disabled', true).val('23:59:59');
                }
            }).trigger('change');

            // Add flatpickr to day_end_time
            flatpickr("#day_end_time", {
                enableTime: true,
                noCalendar: true,
                dateFormat: "H:i:S",
                time_24hr: true,
